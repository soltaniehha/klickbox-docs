...
Whatever framework you're using to build OpenClaw, expose roughly this toolset to the model. Fewer tools beats more tools, but the v1.1 surface has been designed so each tool maps to a distinct User intent on the iPhone.
...
**Attachments** — read-only for agents in v1.x.
...
- **`upload_attachment`** — agents can read existing Attachments via `get_attachment_url` but cannot create new ones in v1.x. The iPhone is the only client that uploads blobs.
...
The smoke test walks the v1.1 tool surface (create with tags, defer/restore, checklist, comments, attachments listing, complete, tag cleanup) and creates a probe Task / Tag prefixed `smoke-probe-`. A SQL clean-up one-liner is in `backend/openclaw-reference/README.md`.
